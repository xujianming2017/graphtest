...
const currentDirName = getRootPathName(currentDir);
...
var flag = true;
var totalPathFilesNum = 0;
var totalParamDataNum = 0;
var path_and_result = {};


describe('收集执行结果', function() {

  it('检查脚本与执行结果', function(done) {
    ...
    files.forEach(function (filename) {
      ...
      // console.info("=========fullname_ext:" + fullname_ext + " filename:" + filename);
      if('.js' == fullname_ext && filename.startsWith('path_')){
        totalPathFilesNum ++;
      } else if('.txt' == fullname_ext){
        if(fullname.endsWith('.result.txt')){
          i++;
        } else {
          totalParamDataNum++;
          txtReferJsName = filename.replace(".txt", ".js");
          path_and_result[txtReferJsName] = filename;
        }
      }
    });
    // console.log("path and param txt:" + JSON.stringify(path_and_result));
    done();
  });
  ...
  it('解析路径执行结果', function(done) {
    if(flag) {
      ...
      files.forEach(function (filename) {
        var fullname = path.join(currentDir, filename);
        var fullname_ext = path.extname(fullname);
        var pathname = path.basename(fullname);
        if('.txt' == fullname_ext && fullname.endsWith('.result.txt')){
          k++;
          fs.readFile(fullname, "utf8", function(err, data){
            if (err) {
              ...
              // 获取pathId
              var pathId = pathname.substring(pathname.indexOf("_")+1, pathname.indexOf("_model_"));
              // console.info("\n解析pathname：" + pathname + " 解析pathId：" + pathId);
              ...
              var invoke_result_path = {
                ...
                pathId: pathId,
                pathFileName: pathname.substring(0, pathname.indexOf(".result.txt")),
                startTime: startTime,
                endTime: endTime,
            ...
            //console.log("===========i:" + i + " totalParamDataNum:" +totalParamDataNum + " totalPathFilesNum:" + totalPathFilesNum + "============\n");
            if(i==totalPathFilesNum){
              done();
            }
          });
        }
  ...
  it('解析路径参数化数据', function(done) {
    if(flag) {
      var files = fs.readdirSync(currentDir);
      var k = 0, i = 0, test_data = "";
      files.forEach(function (filename) {
        var fullname = path.join(currentDir, filename);
        var fullname_ext = path.extname(fullname);
        var pathname = path.basename(fullname);
        if('.js' == fullname_ext && 'recordResult.js' != pathname){
          var pathParamData = currentDir + path.sep + pathname.replace(".js", ".txt");
          var pathParamFileName = path.basename(pathParamData);
          // console.info("从文件名 " + pathname + " 参数化文件：" + pathParamFileName);
          var doParamData = fs.existsSync(pathParamData);
          if(doParamData){
            k++;
            // 读取参数化数据
            fs.readFile(pathParamData, "utf8", function(err, data){
              i++;
              if (err) {
                throw err;
              } else {
                ...
                if(invoke_result_paths.length > 0){
                  ...
                  for(var j=0; j<invoke_result_paths.length; j++){
                    ...
                    // console.info(" 匹配：" + invoke_result_path.pathFileName + ".txt");
                    if((invoke_result_path.pathFileName + ".txt") == pathParamFileName){
                      // console.info(" Match：" + test_data);
                      invoke_result_path.test_data = test_data;
                    }
                  }
                } else {
              ...
              if(i==totalParamDataNum){
                done();
              }
            });

          } else {
            console.log(pathname + "不存在参数化数据!");
          }
        }
      });
      if(k==0){
        console.log("Info: 没有参数化文件！");
        done();
      }
    } else {
      expect(flag).to.be.true;
    ...
    if(flag) {
      if(running_number != "" && running_number != 0 && running_number != 'undefined' && typeof(running_number) != "undefined"){
        ...
        async.each(invoke_result_paths, function(item, callback) {
          ...
          var pathFileName = item.pathFileName;
          delete item["pathFileName"];
          return fetch(rooturl.concat('/api/invoke/create'), fetchRequest(JSON.stringify(item)) ).then(function(res) {
            ...
            item.pathFileName = pathFileName;
      ...
      async.each(invoke_result_paths, function(item, callback) {
        ...
        details.forEach(function (detail) {
          ...
          if(detail.state == 1 || detail.state == 2){
            detail.screenshots = currentDirName + '/screenshots-' + start_exe_time + '-' +  running_number + '/' + item.pathFileName + '_chrome_' + detail.order + '.png';
          }
          delete detail["pathFileName"];
        });
...
function getRootPathName(rootPath){
